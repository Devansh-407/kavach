 ...
 * KavachPay from one-size-fits-all insurance products.
 */
...
export async function calculateDynamicPremium(params: {
  ...
  factors.push({
    ...
    description: loyaltyAdjustment < 0
      ? `You've been with KavachPay for ${weeksSinceOnboarding} weeks with ${totalPastClaims} claims. Loyalty discount earned!`
      : loyaltyAdjustment > 0
