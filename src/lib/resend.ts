...
export async function sendRecoveryEmail(to: string, firstName: string) {
  try {
    const { data, error } = await resend.emails.send({
      from: 'KavachPay AI <onboarding@resend.dev>',
      to: [to],
      subject: 'KavachPay AI - Account Recovery',
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; rounded-lg: 12px;">
          ...
          <p style="color: #475569; font-size: 16px; line-height: 1.6;">We received a request to recover your KavachPay AI account.</p>
          ...
          <p style="color: #94a3b8; font-size: 12px; text-align: center;">© 2026 KavachPay AI. Powered by Guidewire Cloud.</p>
        </div>
      `,
    });
