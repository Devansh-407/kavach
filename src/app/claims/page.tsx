...
import {
  ...
  TrendingUp, RefreshCw, Cpu, ArrowRight, Plus,
} from 'lucide-react';
...
export default function ClaimsPage() {
  ...
  const fetchClaims = async () => {
    const wId = typeof window !== 'undefined' ? localStorage.getItem('kavachpay_workerId') : null;
    if (!wId) {
  ...
  const handleManualClaimSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/claims', {
        ...
        body: JSON.stringify({
          workerId,
          description: manualClaimForm.description,
          triggerType: manualClaimForm.triggerType,
          claimAmount: manualClaimForm.claimAmount,
        }),
      });
      ...
      if (data.success) {
        ...
        setManualClaimForm({ description: '', triggerType: 'Heavy Rain', claimAmount: 500 });
        await fetchClaims();
      } else {
        alert(data.error || 'Failed to create claim');
      }
    } catch {
      alert('Network error creating claim');
    } finally {
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 space-y-4">
        ...
        <Button asChild className="hover:scale-105 transition-transform"><Link href="/onboarding">Get Protected</Link></Button>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="px-6 lg:px-12 h-16 flex items-center justify-between border-b bg-background/50 backdrop-blur-md sticky top-0 z-50">
        ...
        <div className="flex items-center gap-2">
          <Button 
            ...
            className="rounded-xl gap-2 font-bold text-xs hover:scale-105 transition-transform border-primary text-primary"
          >
            <Plus className="h-3.5 w-3.5" /> Add Manual Claim
          </Button>
          <Button 
            variant="default" 
            size="sm" 
            onClick={runAutoScan} 
            disabled={scanning}
            className="rounded-xl gap-2 font-black text-xs uppercase tracking-widest bg-primary hover:bg-primary/90 hover:scale-105 transition-transform"
          >
            {scanning ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Cpu className="h-3.5 w-3.5" />}
            {scanning ? 'Scanning...' : 'Run Auto-Scan'}
          </Button>
          <Button variant="outline" size="sm" asChild className="rounded-xl gap-2 font-bold text-xs hover:scale-105 transition-transform">
          ...
          <CardContent className="p-8 pt-0">
            {claims.length > 0 ? (
                      ...
                      <div className="flex items-center gap-2 justify-end">
                        ...
                        {claim.upiPayoutUrl && (
                          <Button
                            size="sm"
                            variant="default"
                            className="h-6 px-2 text-[9px] font-black uppercase rounded-full hover:scale-105 transition-transform"
              ...
              <div className="text-center py-20 space-y-6">
                ...
                <Button onClick={runAutoScan} disabled={scanning} className="rounded-xl gap-2 font-black text-xs uppercase hover:scale-105 transition-transform">
      ...
      <Dialog open={manualClaimOpen} onOpenChange={setManualClaimOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="font-black font-headline">Add Manual Claim</DialogTitle>
            <DialogDescription>
              Report a disruption that wasn&apos;t automatically detected.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleManualClaimSubmit} className="space-y-4 pt-4">
            <div className="space-y-2">
              <Label htmlFor="triggerType">Disruption Type</Label>
              <Select
                value={manualClaimForm.triggerType}
                onValueChange={(value) => setManualClaimForm({ ...manualClaimForm, triggerType: value })}
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="Poor Air Quality">Poor Air Quality</SelectItem>
                  <SelectItem value="Flood Alert">Flood Alert</SelectItem>
                  <SelectItem value="Urban Curfew">Urban Curfew</SelectItem>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                id="description"
                placeholder="Describe what happened (e.g., 'Heavy rain at 2pm made it impossible to deliver')"
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                min={100}
                max={2000}
                value={manualClaimForm.claimAmount}
                onChange={(e) => setManualClaimForm({ ...manualClaimForm, claimAmount: parseInt(e.target.value) || 500 })}
                required
              />
            </div>
            <div className="flex gap-2 pt-4">
              ...
              <Button
                type="submit"
                disabled={manualClaimLoading || !manualClaimForm.description}
