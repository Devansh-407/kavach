...
import { LogIn, Loader2, Eye, EyeOff, UserPlus, KeyRound, ArrowLeft, MailCheck } from 'lucide-react';
...
export default function LoginPage() {
    ...
    try {
      ...
      if (data.success) {
        if (typeof window !== 'undefined' && data.worker) {
          localStorage.setItem('kavachpay_workerId', data.workerId);
          localStorage.setItem('kavachpay_workerFirstName', data.worker.firstName || '');
          localStorage.setItem('kavachpay_workerLastName', data.worker.lastName || '');
          localStorage.setItem('kavachpay_workerEmail', data.worker.email || '');
          localStorage.setItem('kavachpay_workerPhone', data.worker.phone || '');
        }
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden flex items-center justify-center p-4">
      ...
      <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_20%_20%,rgba(152,251,152,0.08),transparent_50%),radial-gradient(circle_at_80%_80%,rgba(60,179,113,0.05),transparent_50%)] -z-10"></div>
        ...
        <div className="text-center space-y-4">
          ...
          <div className="space-y-1">
            <h2 className="text-2xl font-black text-primary font-headline tracking-tight">
              {mode === 'login' ? 'Welcome Back' : 'Join KavachPay'}
            </h2>
          ...
          <CardContent className="p-8 md:p-10">
            {showForgotPassword ? (
               <div className="space-y-6">
                <Button variant="ghost" className="p-0 h-auto hover:bg-transparent text-primary font-bold flex items-center gap-2 mb-4 hover:scale-105 transition-transform" onClick={() => setShowForgotPassword(false)}>
                ...
                {forgotSuccess ? (
                   <div className="text-center space-y-6 animate-in zoom-in duration-500">
                      ...
                      <Button className="w-full h-14 rounded-2xl font-black shadow-lg hover:scale-105 transition-transform" onClick={() => setShowForgotPassword(false)}>
                        Return to Sign In
                      </Button>
                   </div>
                ) : (
                  <form onSubmit={handleForgotPassword} className="space-y-6">
                    ...
                    <Button type="submit" className="w-full h-14 rounded-2xl font-black shadow-xl hover:scale-105 transition-transform" disabled={forgotLoading}>
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-6">
                  ...
                  <Button variant="outline" className="w-full h-14 rounded-2xl border-none bg-muted/30 hover:bg-primary/5 font-bold transition-all hover:scale-105" onClick={() => { setMode(mode === 'login' ? 'register' : 'login'); setError(''); setSuccess(''); }}>
