...
import { LogIn, Loader2, Eye, EyeOff, Lock, User } from 'lucide-react';
    ...
    try {
      ...
      if (data.success) {
        ...
        if (typeof window !== 'undefined') {
          localStorage.setItem('kavachpay_adminUser', data.admin.username);
        }
        router.push('/admin');
      } else {
      ...
      <div className="w-full max-w-md space-y-8 relative z-10">
        ...
        <div className="text-center space-y-3">
          ...
          <div>
            ...
            <p className="text-white/50 text-sm">KavachPay AI — Insurer Analytics Access</p>
          </div>
        </div>
        ...
        <p className="text-center text-xs text-white/20">
          Protected by KavachPay AI Security
        </p>
      </div>
    </div>
  );
