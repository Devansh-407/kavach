...
import { Send, ChevronLeft, Loader2, Bot, User } from 'lucide-react';
...
export default function ChatPage() {
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', content: "Namaste! I'm your KavachPay AI assistant. How can I help you with your insurance today?" }
  ]);
          ...
          <CardContent className="flex-1 p-0 flex flex-col overflow-hidden">
                ...
                {isLoading && (
                  <div className="flex justify-start">
                    <div className="flex gap-3 items-center text-muted-foreground text-xs bg-muted/50 px-3 py-2 rounded-full">
                      <Loader2 className="h-3 w-3 animate-spin" /> KavachPay AI is thinking...
                    </div>
                  </div>
                )}
              </div>
            ...
            <form onSubmit={handleSendMessage} className="p-4 border-t flex gap-2 bg-white">
              <Input
                ...
                disabled={isLoading}
              />
              <Button type="submit" size="icon" className="rounded-full h-12 w-12 shrink-0 shadow-lg hover:scale-110 transition-transform" disabled={isLoading || !input.trim()}>
